...
build_type="release"
tag="hanoip_${commit_hash:0:7}_$(date +%Y%m%d)"
...
if [[ $? -eq 0 ]]; then
	...
	commit_link=$(cat <<EOF
[${commit_text}](https://github.com/MoeKernel/android_kernel_motorola_hanoip/commit/${commit_hash})
EOF
)
...
EOF
    ...
    build_info=$(cat <<EOF
*hanoip build (#${build_count}) has succeeded*
*Kernel Version*: ${kernel_version}
*Build Type*: \`${build_type}\` *(Fourteen)*
*Tag*: \`${tag}\`

*Duration*: ${elapsed_minutes} Minutes ${elapsed_seconds} Seconds

@MoeKernel #hanoip #ksu
EOF
)
